...
/**
 * @swagger
 * /get/soccer/scoreboard:
 *   get:
 *     summary: Cross-league scoreboard for a given date
 *     tags: [Soccer Data]
 *     security: []
 *     parameters:
 *       - in: query
 ...
 *     responses:
 *       200:
 *         description: Matches grouped by league for the requested date
 */
router.get('/scoreboard', async(req, res) => {
    try {
        ...
        const leagueMap = new Map(leagues.map(l => [l.slug, l]));
        ...
        for (const match of matches) {
            ...
            if (!grouped[slug]) {
                grouped[slug] = {
                    league: leagueMap.has(slug) ? serializeLeague(leagueMap.get(slug)) : { slug, name: slug },
                    events: []
                };
            }
            grouped[slug].events.push(serializeScoreboardEvent(match));
        }
 ...
 *         name: league
 *         schema: { type: string }
 *         description: Optional league slug to filter results
 *     responses:
 *       200:
 *         description: Matching clubs (max 20)
 *       400:
...
router.get('/clubs/search', async(req, res) => {
    try {
        ...
        if (req.query.league) {
            const leagueSlug = String(req.query.league).toLowerCase();
            if (!LEAGUE_PATTERN.test(leagueSlug)) {
                return res.status(400).json({ error: 'Invalid league slug' });
            }
            filter.league_slugs = leagueSlug;
        }

        const clubs = await SoccerClub.find(filter)
            .select('-roster -coach -source_payload')
            .sort({ display_name: 1 })
            .limit(20)
            .lean();
...
/**
 * @swagger
 * /get/soccer/leagues/search:
 *   get:
 *     summary: Search leagues by name or abbreviation
 *     tags: [Soccer Data]
 *     security: []
 *     parameters:
 *       - in: query
 ...
 *     responses:
 *       200:
 *         description: Matching leagues (max 10)
 *       400:
 *         description: Missing or too-short query
 */
